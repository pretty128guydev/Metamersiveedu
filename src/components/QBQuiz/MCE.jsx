...
function MultiChoiceExtraQuiz(props) {
  const { index, level, data, handleSelect, updateData, selected, isEditable, teacher_id } = props;
  ...
  useEffect(() => {
    ...
    const subQuestion = document.getElementById(`subQuestion${index}`);
    ...
    subQuestion.value = props.data.subQuestion ? props.data.subQuestion : '';
  ...
  const handleSave = async (event) => {
    ...
    subQuizList.forEach((_, subQuizId) => {
      body.append('subQuestions', event.target[`MC_Quiz${index}SubQuiz${subQuizId}`].value);
    });

    body.append('type', 6);
    body.append('question', event.target[`question${index}`].value);
    body.append('subQuestion', event.target[`subQuestion${index}`].value);
    ...
    <Card className={"mb-3 multiple-choice" + (selected ? " selected" : "")}>
      <form className="was-validated" onSubmit={handleSave}>
        <CardHeader className="d-flex justify-content-between align-items-center">
          <span className="m-2 fa-lg">{index + 1}. {translate("multiple-choice-extra")}</span>
          {
            isEditable !== false &&
            <div>
              {loading ? (
        ...
        <CardBody>
          ...
          >
            <div className="w-100 mb-3">
              <input
                type="text"
                className="form-control bg-white bg-opacity-5 is-invalid mb-2"
              ...
              <textarea
                ...
                placeholder="Type the sub question here."
          ...
          >
            <div className="w-100 mb-3">
              ...
              <pre style={{ background: 'transparent', color: 'white', border: 'none' }}>{quiz.subQuestion}</pre>
              <span>{quiz.point} {translate("point")}</span>
            </div>
