...
const WordDash = () => {
  ...
  useEffect(() => {
    const fetchStudentData = async () => {
      ...
      try {
        ...
        const res = await WordApi.getJoinedClassrooms(query);
    ...
    const fetchTeacherData = async () => {
      setLoading(true);
      try {
        const res = await WordApi.getClassroomsByTeacherId({
          teacher_id: userInfo.uid,
        });
        setTableData(res.data.ret);
        ...
        notification.error({
          message: "Error",
          description:
            error.response?.data?.message || "An error occurred fetching data",
        });
      } finally {
        setLoading(false);
      }
    ...
    // Cleanup function to reset layout on component unmount
    return () => {
  ...
  const handleAddStudent = () => {
    ...
    if (!exists) {
      WordApi.joinClassroom(addSelectedStudent)
        .then((res) => {
  ...
  const handleAddStudentByGmail = () => {
    ...
    if (!exists) {
      WordApi.joinClassroom(addedStudentByGmail)
        .then((res) => {
          let tmpTableData = tableData;
          console.log("Start => ", tmpTableData, selectedTable);
          for (let i = 0; i < tmpTableData.length; i++) {
  ...
  const handleAddClass = (event) => {
    ...
    if (userInfo.permission.word === false) {
      alert(
    ...
    const respTime = event.target.time.value;
    const wordCount = event.target.count.value;
  ...
  const handleDeleteClass = () => {
    WordApi.deleteClassroom({ class_id: selectedTable.id })
      .then((data) => {
